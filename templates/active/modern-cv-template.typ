...
#let resume(
  ...
  let contact-item(value, prefix: "", link-type: "", display: none) = {
    if value != "" {
      let label = if display == none { value } else { display }
      if link-type != "" {
        link(link-type + value)[#(prefix + label)]
      } else {
        label
      }
    }
    ...
    align(personal-info-position)[
      #{
        let items = (
          ...
          contact-item(github, link-type: "https://github.com/", display: github),
          contact-item(linkedin, link-type: "https://linkedin.com/in/", display: "in/" + linkedin),
...
#let edu(
  ...
  [
    #strong(degree)#if program != "" [#strong[ in #program]]#if institution != "" [ \@ #institution]#if (
      gpa != ""
    ) [ — (GPA: #gpa)] #h(1fr) #emph(dates) \
    #if courses.len() > 0 [
      #v(-0.5em)
